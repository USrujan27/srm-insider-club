...
export default function AboutPage() {
  ...
  return (
    <main className="min-h-screen pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto selection:bg-indigo-500/30">
      ...
      >
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass-pill mb-6">
          <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse" />
          <span className="text-xs font-medium text-indigo-200">Our Mission</span>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-32">
        {stats.map((stat, i) => (
          <motion.div
            ...
            className="glass-card liquid-hover p-8 text-center"
          >
            <div className="text-4xl md:text-5xl font-bold text-white mb-2 tracking-tighter drop-shadow-lg">{stat.value}</div>
            <div className="text-sm font-medium text-[#A1A1AA] uppercase tracking-widest">{stat.label}</div>
          </motion.div>
        ))}
      </section>
      ...
      <section>
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tighter mb-4">Core Values</h2>
          <p className="text-[#A1A1AA] max-w-2xl mx-auto">The principles that guide our editorial team and platform development.</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {values.map((value, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: i * 0.1 }}
              className="glass-card liquid-hover p-8 relative overflow-hidden group"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <div className="w-12 h-12 rounded-full glass flex items-center justify-center mb-6 border border-white/5 shadow-[0_0_15px_rgba(99,102,241,0.2)]">
                <span className="text-indigo-400 font-bold text-xl">{i + 1}</span>
              </div>
              <h3 className="text-xl font-bold text-white mb-3 tracking-snug">{value.title}</h3>
